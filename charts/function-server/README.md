...
**Note:** unlike the other app repos in this org, `function-server`'s
`.github/workflows/image.yml` currently only builds `:latest` on push to
`main` - it doesn't tag versioned releases or send the
`function-server-released` repository_dispatch that `bump-chart.yaml`
(generic across every chart, see the root `CLAUDE.md`) listens for. Until
that's added there, this chart's `version`/`appVersion` won't auto-bump and
`image.tag` defaults to `latest` accordingly (see below) rather than a
pinned version.
